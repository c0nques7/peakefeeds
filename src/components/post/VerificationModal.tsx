'use client'

import { useState, useEffect } from 'react';
import { useAccount, useConnect, useSignMessage } from 'wagmi';
import { Wallet, PlayCircle, Loader2, ShieldCheck, AlertCircle } from 'lucide-react';
import { useAdMediator } from '@/hooks/useAdMediator';
import { AdPlayerOverlay } from '@/components/ads/AdPlayerOverlay';
import { toast } from "sonner"; 
import { createPortal } from 'react-dom'; // 🆕 CRITICAL PORTAL FIX

interface VerificationModalProps {
  isOpen: boolean;
  onClose: () => void;
  onVerified: (method: 'WALLET' | 'AD' | 'SKIP', signature?: string) => void;
  contentHash: string; 
}

export function VerificationModal({ isOpen, onClose, onVerified, contentHash }: VerificationModalProps) {
    const { address, isConnected } = useAccount();
    const { connectors, connectAsync } = useConnect();
    const { signMessageAsync } = useSignMessage();
    
    // Ad and Mediator State
    const { triggerAdWaterfall, status: adStatus, currentProvider } = useAdMediator();
    
    // Component State
    const [isSigning, setIsSigning] = useState(false);
    const [mounted, setMounted] = useState(false); // For Portal check

    // 1. Mount and State Reset
    useEffect(() => {
        setMounted(true);
        if (!isOpen) {
            setIsSigning(false);
        }
    }, [isOpen]);

    // If not mounted or closed, return null (preventing server-side render issues)
    if (!mounted || !isOpen) return null;


    // --- OPTION A: WALLET (Direct Pay) ---
    const handleWalletVerify = async () => {
        setIsSigning(true);
        try {
            let currentAddress = address;
            if (!isConnected || !currentAddress) {
                const result = await connectAsync({ connector: connectors[0] });
                currentAddress = result.accounts[0];
            }

            if (currentAddress) {
                const signature = await signMessageAsync({ 
                    // Using Viem/Ethers style raw message signing
                    message: { raw: contentHash as `0x${string}` } 
                });

                if (signature) {
                    onVerified('WALLET', signature);
                }
            }
        } catch (error) {
            console.error("Wallet verification failed:", error);
            toast.error("Signature rejected or connection failed.");
        } finally {
            setIsSigning(false);
        }
    };

    // --- OPTION B: AD (Relayed) ---
    const handleWatchAd = async () => {
        setIsSigning(true); // Sign state for the initial signature (UX)
        try {
            let currentAddress = address;
            
            // 1. Ensure Wallet Connection
            if (!isConnected || !currentAddress) {
                const result = await connectAsync({ connector: connectors[0] });
                currentAddress = result.accounts[0];
            }

            // 2. Sign Message FIRST (Proof of Ownership for the Relayer)
            const signature = await signMessageAsync({ 
                message: { raw: contentHash as `0x${string}` }
            });

            setIsSigning(false); // Done signing, entering Ad mode

            // 3. Trigger the Ad Waterfall
            // The AdPlayerOverlay becomes visible here due to adStatus changing
            await triggerAdWaterfall('peake-ad-container', {
                userId: currentAddress!,
                contentHash: contentHash,
                signature: signature
            });

            // 4. If triggerAdWaterfall resolves, it means ad completed successfully
            onVerified('AD', signature);

        } catch (error: any) {
            setIsSigning(false);
            if (error.message.includes('User rejected')) {
                toast.warning("Verification requires signing the content.");
            } else {
                toast.error("Ad service failed. Please try Wallet or Skip.");
            }
        }
    };


    // The main JSX content for the modal (rendered inside the Portal)
    const modalContent = (
        <div className="w-full max-w-md bg-[var(--bg-app)] border border-[var(--glass-border)] rounded-3xl p-6 shadow-2xl relative overflow-hidden">
            {/* Header */}
            <div className="text-center mb-6">
                <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-emerald-500/20 mb-4 text-emerald-400 border border-emerald-500/30 shadow-[0_0_30px_rgba(16,185,129,0.2)]">
                    <ShieldCheck size={32} />
                </div>
                <h2 className="text-2xl font-bold text-[var(--text-primary)]">Verify Content</h2>
                <p className="text-[var(--text-muted)] text-sm mt-2">
                    Anchor this post to the Optimism Blockchain to prove authenticity.
                </p>
            </div>

            {/* --- THE CHOICE --- */}
            <div className="space-y-3" style={{ opacity: (adStatus === 'SHOWING' || adStatus === 'LOADING') ? 0.2 : 1 }}>
                
                {/* Button A: Wallet */}
                <button 
                    onClick={handleWalletVerify}
                    disabled={isSigning || adStatus !== 'IDLE'}
                    className="w-full flex items-center justify-between p-4 rounded-xl bg-indigo-600/10 border border-indigo-500/30 hover:bg-indigo-600 hover:text-white text-indigo-400 transition-all group disabled:opacity-50"
                >
                    <div className="flex items-center gap-4">
                        <div className="bg-indigo-500/20 p-2 rounded-lg group-hover:bg-white/20"><Wallet size={24} /></div>
                        <div className="text-left">
                            <span className="block font-bold text-sm">Connect Wallet</span>
                            <span className="block text-[10px] opacity-70">Pay Gas (~$0.01 ETH)</span>
                        </div>
                    </div>
                    {isSigning ? <Loader2 className="animate-spin" /> : <span>→</span>}
                </button>

                {/* Button B: Ad */}
                <button 
                    onClick={handleWatchAd}
                    disabled={isSigning || adStatus !== 'IDLE'}
                    className="w-full flex items-center justify-between p-4 rounded-xl bg-[var(--glass-card)] border border-[var(--glass-border)] hover:bg-[var(--glass-card-hover)] hover:border-emerald-500/30 transition-all group disabled:opacity-50"
                >
                    <div className="flex items-center gap-4">
                        <div className="bg-[var(--glass-panel)] p-2 rounded-lg group-hover:bg-emerald-500/20 group-hover:text-emerald-400 text-[var(--text-primary)]"><PlayCircle size={24} /></div>
                        <div className="text-left">
                            <span className="block font-bold text-[var(--text-primary)]">Watch to Verify</span>
                            <span className="block text-[10px] text-[var(--text-muted)]">We pay the gas fees for you.</span>
                        </div>
                    </div>
                    {adStatus === 'LOADING' ? <Loader2 className="animate-spin text-emerald-500" /> : null}
                </button>

                <div className="py-2 flex items-center justify-center">
                    <span className="text-[10px] text-[var(--text-muted)] uppercase tracking-widest">or</span>
                </div>

                {/* Button C: SKIP */}
                <button 
                    onClick={() => onVerified('SKIP')}
                    className="w-full flex items-center justify-center gap-2 py-3 rounded-xl text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[var(--glass-card-hover)] transition-colors text-xs"
                >
                    <AlertCircle size={14} />
                    Post without verification (Unverified)
                </button>

            </div>
        </div>
    );


    // --- FINAL PORTAL RENDER ---
    return createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/90 backdrop-blur-sm p-4">
            
            {/* The Ad Overlay Component is rendered here, handling the full screen video */}
            <AdPlayerOverlay 
                status={adStatus} 
                provider={currentProvider} 
                onCancel={onClose} 
            />
            
            {/* The main choice modal */}
            {modalContent}

        </div>,
        document.body // Renders as a direct child of the <body> tag
    );
}

